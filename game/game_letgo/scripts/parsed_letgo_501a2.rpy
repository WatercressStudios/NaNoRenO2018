label letgo_501a2:
    scene letgo park podium with dissolve
    play ambience "game_letgo/ambience/Park Day.ogg" fadeout 2.0 fadein 2.0
    show eli goofy1 with dissolve:
        align (0.35, 1.0)
    # VA teasingly
    voice "C-501a2-1.mp3" #Elijah (Michael Potok)
    eli "I didn’t know you were {i}this{/i} into parks, May."
    show may confused1 with dissolve:
        align (0.65, 1.0)
    voice "C-501a2-2.mp3" #Maya (shiena)
    may "What do you mean? We used to love this place!"

    voice "C-501a2-3.mp3" #Elijah (Michael Potok)
    eli "Yeah, in spring. Or summer. Or even fall. We don’t come here during winter."
    show may nervous2 with dissolve
    voice "C-501a2-4.mp3" #Maya (shiena)
    may "We were just here--"
    show may sad1 with dissolve
    "A look of surprise flashes across her face - only to be replaced by a small, bittersweet smile."
    show eli worried2 with dissolve
    voice "C-501a2-5.mp3" #Elijah (Michael Potok)
    eli "May?"
    show may normal1 with dissolve
    show eli normal1 with dissolve
    voice "C-501a2-6.mp3" #Maya (shiena)
    may "Nothing. I was just reliving some good memories."
    show eli smile1 with dissolve
    show may happy2 with dissolve
    "I gently squeeze her hand and look around. The swingset by the tree, the ducks by the pond, the bench beneath the blue sky…"

    "This is perfect, just as I remember - and that perfection is only compounded by her warm hand."
    show eli smile2 with dissolve
    voice "C-501a2-7.mp3" #Elijah (Michael Potok)
    eli "We’ve made quite a few of those, haven’t we?"
    show may frown1 with dissolve
    voice "C-501a2-8.mp3" #Maya (shiena)
    may "You don’t know the half of it."

    "She said that so softly I barely heard it."
    show may forcedsmile1 with dissolve
    voice "C-501a2-9.mp3" #Maya (shiena)
    may "Thank you, Eli."
    show eli worried1 with dissolve
    voice "C-501a2-10.mp3" #Elijah (Michael Potok)
    eli "Huh? Sure… I mean, thank you, May."

    "Something’s different about us. We’re normally so much more chatty than this. What happened to the usual back-and-forth teasing?"
    "Is it because we’re a couple now that we’re acting so strangely?"

    "Desperate to change the mood, I spot a couple of ducks huddled together by the pond."

    show eli normal1 with dissolve

    voice "C-501a2-11.mp3" #Elijah (Michael Potok)
    eli "Uhh, so… how about we get some bread to feed--"

    show may angry1 with dissolve
    voice "C-501a2-12.mp3" #Maya (shiena)
    may "No, let’s not."

    voice "C-501a2-13.mp3" #Elijah (Michael Potok)
    eli "Huh?"
    show may happy2 with dissolve
    # VA: happily
    voice "C-501a2-14.mp3" #Maya (shiena)
    may "Let’s do something different this time… I want to make a new memory!"

    show eli smile2 with dissolve
    voice "C-501a2-15.mp3" #Elijah (Michael Potok)
    eli "Oh?"
    show may challenge1 with dissolve
    voice "C-501a2-16.mp3" #Maya (shiena)
    may "Don’t you \"oh\" me. Help me think of something fun!"
    show eli normal1 with dissolve
    voice "C-501a2-17.mp3" #Elijah (Michael Potok)
    eli "But what about the cute baby ducklings? What if they’re starving?"
    show may suspicious1 with dissolve
    voice "C-501a2-18.mp3" #Maya (shiena)
    may "They {i}never{/i} starve - have you seen how much people feed them!?"

    "I grin at her. I guess I was worried for nothing and we haven’t changed at all. The chemistry’s still there."

    show eli cool1 with dissolve

    voice "C-501a2-19.mp3" #Elijah (Michael Potok)
    eli "What did you have in mind?"

    "She cups her chin in her gloved hand and takes a moment to think. She’s {i}actually{/i} giving this some serious thought!"
    "I can’t help but to chuckle at her… What’s gotten into Maya?"
    show eli cool1 with dissolve
    voice "C-501a2-20.mp3" #Elijah (Michael Potok)
    eli "You know, there’s no pressure to do anything different today. We have the rest of our lives, after all."

    #a summoner has disconnected
    show may forcedsmile1 with dissolve
    voice "C-501a2-21.mp3" #Maya (shiena)
    may "..."

    voice "C-501a2-22.mp3" #Maya (shiena)
    may "...Yeah."

    "I feel my cheeks burn. Oh shit, did I just say that? Did I really just declare that I..."
    show eli worried1 with dissolve
    voice "C-501a2-23.mp3" #Elijah (Michael Potok)
    eli "I’m not saying-- What I mean is..."

    "Wait, why am I backtracking? We’re a couple now. I can finally say these things out loud!"
    show eli cool1 with dissolve
    voice "C-501a2-24.mp3" #Elijah (Michael Potok)
    eli "What I mean is, I want us to grow old together, May."

    show may happy2 with dissolve
    show may:
        ease 0.5 align (0.55,1.0)
    "Maya laughs and rests her head against my arm."
    show may aww2 with dissolve
    voice "C-501a2-25.mp3" #Maya (shiena)
    may "Oh Eli, you’re such a dork."

    show may forcedsmile1 with dissolve
    voice "C-501a2-26.mp3" #Maya (shiena)
    may "And yes… I... want to grow old together, too..."
    show eli worried1 with dissolve
    "There’s a strain in her voice - but I’m not sure why. Before I can ask her about it, she suddenly pulls back."

    show may:
        ease 0.5 align (0.6,1.0)
    pause 0.5
    show may shout1 with dissolve
    # VA: gasp
    voice "C-501a2-27.mp3" #Maya (shiena)
    stop music fadeout 1.0
    may "Ah! Grow old together! I have an idea! Come on, Eli!"

    show may:
        ease 0.5 align (0.80,1.0)
    pause 0.1
    show eli:
        ease 0.5 align (0.55,1.0)

    "Her grip suddenly tightens and she starts pulling me away."
    show eli worried1 with dissolve
    voice "C-501a2-28.mp3" #Elijah (Michael Potok)
    eli "M-May!?"

    show may happy2 with dissolve
    voice "C-501a2-29.mp3" #Maya (shiena)
    may "Hurry up, we can still make it!"
    "Whoa! She goes into a sprint. Still make what!?"

    jump letgo_501a3
