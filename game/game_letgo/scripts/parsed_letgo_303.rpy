label letgo_303:
    scene letgo downtown cafe outside with dissolve

    stop music fadeout 3.0

    show eli smile2:
        xanchor 0.5
        yalign 1.0
        xpos 0.6

    show may angry1:
        xanchor 0.5
        yalign 1.0
        xpos 0.3

    with dissolve

    voice "C-303-1.mp3" #Maya (shiena)
    may "I’m fine. This isn’t about me."

    "Huh?"

    voice "C-303-2.mp3" #Elijah (Michael Potok)
    eli "Not about you?"

    show eli sad2 with dissolve

    voice "C-303-3.mp3" #Elijah (Michael Potok)
    eli "Getting a soda spilled on me isn’t the end of the world, May."

    show eli:
        easein 1.1 xpos 0.42

    "I take her hand."

    play music bgmloop2death_intro noloop fadeout 1.0
    queue music bgmloop2death_loop loop

    show eli smile2 with dissolve

    voice "C-303-4.mp3" #Elijah (Michael Potok)
    eli "This date’s for both of us, right? You should be enjoying yourself, too."

    show may angry2 close with dissolve

    voice "C-303-5.mp3" #Maya (shiena)
    may "That is {i}not--{/i}"

    voice "C-303-6.mp3" #Elijah (Michael Potok)
    eli "C’mon! You’re tense. Why not just sit down for a bit while I get you the best hot chocolate that five bucks can buy?"

    show may angry2 with dissolve

    voice "C-303-7.mp3" #Maya (shiena)
    may "No. We need to keep moving."

    voice "C-303-8.mp3" #Elijah (Michael Potok)
    eli "Moving? To where?"

    show may worried2 with dissolve

    voice "C-303-9.mp3" #Maya (shiena)
    may "I..."

    show may sad1 with dissolve

    voice "C-303-10.mp3" #Maya (shiena)
    may "I don’t know."

    show eli cool1 with dissolve

    voice "C-303-11.mp3" #Elijah (Michael Potok)
    eli "Let’s just calm down and take things one step at a time, alright?"

    #breathing
    voice "C-303-12.mp3" #Maya (shiena)
    may "..."

    show may sadshout1 with dissolve

    voice "C-303-13.mp3" #Maya (shiena)
    may "Alright."

    "I squeeze her hand for a moment before letting go."

    show eli:
        easein 0.6 xpos 0.5

    voice "C-303-14.mp3" #Elijah (Michael Potok)
    eli "Alright. I’ll see you in a few minutes, May."

    voice "C-303-15.mp3" #Maya (shiena)
    may "Unless you die."

    voice "C-303-16.mp3" #Elijah (Michael Potok)
    eli "Nah."

    voice "C-303-17.mp3" #Elijah (Michael Potok)
    eli "I’d come back as a zombie just to make sure you get that hot chocolate."

    show may aww1 close with dissolve

    voice "C-303-18.mp3" #Maya (shiena)
    may "Hm. Romantic."

    show eli:
        ease 0.8 align (0.0, 1.0) alpha 0

    scene letgo mall2
    play ambience "game_letgo/ambience/Mall.ogg" fadeout 0.1 fadein 0.1

    show eli normal1 at flip:
        xanchor 0.5
        yalign 1.0
        xpos 0.5
        ypos 1.15
    with dissolve

    "I wave goodbye with my free hand and disappear into the mall."

    scene letgo mall1
    play ambience "game_letgo/ambience/Mall.ogg" fadeout 0.1 fadein 0.1

    show eli normal1:
        xanchor 0.5
        yalign 1.0
        xpos 0.7
    with dissolve

    "I check the brand to make sure I stop by the same coffee shop May went to."

    scene letgo downtown cafe inside
    with dissolve

    "It turns out \"the best hot chocolate five dollars can buy\" is pretty extravagant."

    play ambience "game_letgo/ambience/Downtown Night.ogg" fadeout 0.1 fadein 0.1

    scene letgo downtown cafe outside with dissolve

    show may sad1 at flip:
        xanchor 0.5
        yalign 1.0
        xpos 0.6
    with dissolve

    pause 0.5

    show eli smile1 at flip behind may:
        xanchor 0.5
        yalign 1.0
        xpos 0.3

    with easeinleft

    "I quickly make it back and offer the drink in my left hand."

    voice "C-303-19.mp3" #Elijah (Michael Potok)
    eli "Here."

    show eli at flip:
        easein 0.8 xpos 0.47

    "She glances at the cup as we start walking."

    voice "C-303-20.mp3" #Maya (shiena)
    may "Alright."

    "She grabs it and takes a sip."

    "Bingo!"

    voice "C-303-21.mp3" #Maya (shiena)
    may "It’s... kind of tepid?"

    voice "C-303-22.mp3" #Elijah (Michael Potok)
    eli "And more than that, you just up and poisoned yourself."

    show may worried1 with dissolve

    voice "C-303-23.mp3" #Maya (shiena)
    may "{i}What.{/i}"

    show eli cool1 with dissolve

    voice "C-303-24.mp3" #Elijah (Michael Potok)
    eli "You said you’d try to poison me - but when I realized the hot chocolate was your plan, I thought up this counter-plan to have you drink it yourself!"

    show may angry1 close with dissolve

    voice "C-303-25.mp3" #Maya (shiena)
    may "Oooooh, back to that again. Okay."

    show may confused1 with dissolve

    voice "C-303-26.mp3" #Maya (shiena)
    may "Clever."

    show eli sad2 with dissolve

    voice "C-303-27.mp3" #Elijah (Michael Potok)
    eli "...That’s it?"

    voice "C-303-28.mp3" #Elijah (Michael Potok)
    eli "C’mon, May. Normally you’d say that you’ve been taking small doses of the poison since you were a kid, and now you’re immune to it."

    voice "C-303-29.mp3" #Elijah (Michael Potok)
    eli "And then when I sipped the other drink, you’d say you bribed the coffee shop so they’d poison anything I bought from them."

    show may aww1 close with dissolve

    voice "C-303-30.mp3" #Maya (shiena)
    may "Heh..."

    "Hm. Not taking the bait, huh?"

    "I sigh."

    voice "C-303-31.mp3" #Elijah (Michael Potok)
    eli "Here. This one’s warmer."

    show eli at flip:
        easein 0.8 xpos 0.485
    show eli at flip:
        easein 0.8 xpos 0.47

    "I swap the drinks and she takes a sip."

    show may sad1 with dissolve

    voice "C-303-32.mp3" #Maya (shiena)
    may "Mm. Yeah, that’s better."

    "...But that didn’t really cheer her up either."

    hide may
    show may sad1:
        xanchor 0.5
        yalign 1.0
        xpos 0.6
    with dissolve

    hide may
    hide eli
    with easeoutright

    scene black with dissolve

    pause 0.5

    scene letgo downtown night

    show may sad1:
        xanchor 0.5
        yalign 1.0
        xpos 0.75

    show eli worried1:
        xanchor 0.5
        yalign 1.0
        xpos 0.9

    "C’mon. Think, think..."

    voice "C-303-33.mp3" #Elijah (Michael Potok)
    eli "Hey."

    voice "C-303-34.mp3" #Elijah (Michael Potok)
    eli "Let’s do something big next weekend!"

    voice "C-303-35.mp3" #Maya (shiena)
    may "Excuse me?"

    voice "C-303-36.mp3" #Elijah (Michael Potok)
    eli "Yeah. Splurge a bit! Like a ski trip or something!"

    show may angry2 with dissolve

    voice "C-303-37.mp3" #Maya (shiena)
    may "Ugh."

    show eli sad2 with dissolve

    voice "C-303-38.mp3" #Elijah (Michael Potok)
    eli "May?"

    stop music fadeout 5.0

    voice "C-303-39.mp3" #Maya (shiena)
    may "Yeah. Sure. Ski trip. Fine. Because everything’s gonna be okay, and we’ll wrap it all up with a hunky-dory goddamn ski trip."

    "Um."

    show eli smile2 with dissolve

    voice "C-303-40.mp3" #Elijah (Michael Potok)
    eli "Well, we wouldn’t really \"wrap it up\" with the ski trip. There’d be a lot more dates to come after..."

    show may angry2 with dissolve

    voice "C-303-41.mp3" #Maya (shiena)
    may "Oh! Of course! The other dates! The ones that we’ll have in our bright and happy future together."

    voice "C-303-42.mp3" #Elijah (Michael Potok)
    eli "I mean... I’d really hope so."

    voice "C-303-43.mp3" #Maya (shiena)
    may "Yeah, well sometimes, shit happens."

    show eli sad2 with dissolve

    "I can feel a pit in my stomach."

    "What’s she talking about?"

    "Did I do something wrong?"

    "Is she breaking up with me already!?"

    "Why?"

    "I can’t keep dancing around this and trying to cheer her up. If something’s wrong, it’s time to get to the heart of it."

    voice "C-303-44.mp3" #Elijah (Michael Potok)
    eli "Hey."

    voice "C-303-45.mp3" #Elijah (Michael Potok)
    eli "Mind if I ask you something?"

    voice "C-303-46.mp3" #Maya (shiena)
    may "N--"

    show may angry1 close with dissolve

    voice "C-303-47.mp3" #Maya (shiena)
    may "Actually, you know what?"

    voice "C-303-48.mp3" #Maya (shiena)
    may "Fine. Ask away."

    "Alright, here goes."

    voice "C-303-49.mp3" #Elijah (Michael Potok)
    eli "Is something bothering you?"

    voice "C-303-50.mp3" #Maya (shiena)
    may "What do you think?"

    voice "C-303-51.mp3" #Elijah (Michael Potok)
    eli "I’m leaning... yes?"

    show may angry1 with dissolve

    voice "C-303-52.mp3" #Maya (shiena)
    may "Good to see Detective Elijah is as astute as ever."

    voice "C-303-53.mp3" #Elijah (Michael Potok)
    eli "Is there... anything I can do to help?"

    voice "C-303-54.mp3" #Maya (shiena)
    may "Judging by how things have been going, I’m gonna say that if you don’t find a way to fix it in the next thirty seconds, probably not."

    show eli worried2 with dissolve

    voice "C-303-55.mp3" #Elijah (Michael Potok)
    eli "...Uh-huh."

    voice "C-303-56.mp3" #Elijah (Michael Potok)
    eli "Can we... try anyways?"

    voice "C-303-57.mp3" #Maya (shiena)
    may "...Huh?"

    voice "C-303-58.mp3" #Elijah (Michael Potok)
    eli "Who knows? I might surprise you."

    voice "C-303-59.mp3" #Elijah (Michael Potok)
    eli "And if I can’t lend a hand, I can at least offer a shoulder."

    show may worried2 with dissolve

    voice "C-303-60.mp3" #Maya (shiena)
    may "Well..."

    #deep breathin’
    voice "C-303-61.mp3" #Maya (shiena)
    may "..."

    voice "C-303-62.mp3" #Maya (shiena)
    may "Okay."

    voice "C-303-63.mp3" #Maya (shiena)
    may "I’ll... I’ll tell you."

    voice "C-303-64.mp3" #Elijah (Michael Potok)
    eli "Great. I’m all ears, May."

    voice "C-303-65.mp3" #Maya (shiena)
    may "Okay, so the first part is... You’re not gonna believe me."

    show eli goofy1 with dissolve

    voice "C-303-66.mp3" #Elijah (Michael Potok)
    eli "I dunno, I’m pretty gullible."

    show may forcedsmile1 with dissolve

    voice "C-303-67.mp3" #Maya (shiena)
    may "Heh. Good point."

    show may sad1 with dissolve

    voice "C-303-68.mp3" #Maya (shiena)
    may "So I guess the gist of it is that you’ve been... Well..."

    stop ambience fadeout 1.0
    play sound "game_letgo/sfx/Rope Snap.ogg"
    play music bgmloop3comedy_intro noloop fadeout 1.0
    queue music bgmloop3comedy_loop loop

    "Huh?"

    show eli sad1 with dissolve

    voice "C-303-69.mp3" #Elijah (Michael Potok)
    eli "May!"

    show eli:
        easein 0.2 xpos 0.85

    play sound "game_letgo/sfx/Punch,Shove Serious.ogg"

    show may:
        easein 0.2 ypos 1.10
        rotate -10
        ease 0.8 align (0.0, 1.0) alpha 0

    "My eyes peer up, and I immediately shove May off of the sidewalk."

    "I don’t have time to get out of the way - but I do manage to catch a glimpse of the object above me."

    scene black

    #PIANO CRASH SOUND EFFECT BABY THIS IS WHAT I’VE BEEN WAITING FOR
    play sound "game_letgo/sfx/Piano Crash.ogg"

    "A grand piano."

    "..."

    "..."

    voice "C-303-70.mp3" #Maya (shiena)
    may "Ha..."

    voice "C-303-71.mp3" #Maya (shiena)
    may "Hahahaha..."

    voice "C-303-72.mp3" #Maya (shiena)
    may "A piano!"

    voice "C-303-73.mp3" #Maya (shiena)
    may "He got smashed by a piano!"

    voice "C-303-74.mp3" #Maya (shiena)
    may "It’s like a cartoon!"

    voice "C-303-75.mp3" #Maya (shiena)
    may "This whole thing!"

    voice "C-303-76.mp3" #Maya (shiena)
    may "It’s all just a fucking cartoon!"

    voice "C-303-77.mp3" #Maya (shiena)
    may "Hahaha..."

    voice "C-303-78.mp3" #Maya (shiena)
    may "..."

    stop music fadeout 10.0

    voice "C-303-79.mp3" #Maya (shiena)
    may "...I can’t."

    voice "C-303-80.mp3" #Maya (shiena)
    may "All of this, I just..."

    voice "C-303-81.mp3" #Maya (shiena)
    may "I can’t."

    voice "C-303-82.mp3" #Maya (shiena)
    may "I can’t take this anymore."

    play audio "game_letgo/sfx/Disturbing Bass.ogg"
    scene letgo rewind with Dissolve(9.5)

    jump letgo_401
